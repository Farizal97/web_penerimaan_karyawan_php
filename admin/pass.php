...
if (isset($_POST["simpan"])) {
	...
	if ($password_sekarang != $admin["password"]) {
			
	echo "<script>alert(\"Password lama salah!\");</script>";
	echo "<meta http-equiv='refresh' content='1;URL=pass.php?id_admin=$id_admin'>";
	...
	} elseif ($password_sekarang == $admin["password"] && $password_baru_1 == $password_baru_2) {
	
	$sql = "UPDATE admin SET password = '$password_baru_1' WHERE id_admin = '$id_admin'";
	$query = mysqli_query($connect,$sql);
	if (!$query){
		die ('Gagal mengubah password' . mysqli_error());
	}
	
	echo "<script>alert(\"Password admin berhasil diedit!\");</script>";
